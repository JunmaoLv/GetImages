...
from configuration import IMAGE_HEIGHT, IMAGE_WIDTH, CHANNELS, \
    ...
    model_name_list, model_index, EPOCHS, cross_validate_tfrecord, k_fold, CHANNELS
from parse_tfrecord import get_parsed_dataset


def load_and_prepocess_image_from_camera(image):
    print('image standardization')
    image = img_as_float(image)
    # print('max: {}, min: {}'.format(np.max(image), np.min(image)))
    image = tf.image.per_image_standardization(image)
    # print('resize image from {} to {}'.format(image.shape, [IMAGE_HEIGHT, IMAGE_WIDTH, CHANNELS]))
